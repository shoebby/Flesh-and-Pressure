label piss:
    scene ch4
    play audio "chaptercard.mp3"
    pause

    f_t "Its been weeks but I keep thinking about Piper, she's so... Fun."
    f_t "I try to use her at least once a day, it's grown to be my favourite task. Even over FIFOing the drinks and snacks."
    f_t "Days have been feeling more and more monotonous though, the rot is setting in so to speak."
    f_t "Stuff just winds up getting more and more boring the longer I do it."
    f_t "At least there's Piper and her... fun... sounds."

    nvl clear

    m "Heyyy Itske! Can you do a cleaning round on the bathrooms?"

    f "Whuh? I already did one earlier today, can't you do it?"

    m "I've still got some shift leader tasks to finish up, sorry! Pretty please?"

    f_t "That's such bullshit, but I really do not feel like arguing... I guess it's something to do."

    f "... Okay fine. Please don't make this a habit though, okay?"

    m "Yaaay thank you, and I won't! Don't worry!"

    f_t "Mae's cute, her indestructible sense of optimism is refreshing. I haven't really met many people like that over the past few years."
    f_t "Mostly people who used to be like that I guess, people who turned into something different by the world around them."
    f_t "By force more than anything."
    f_t "It's sad, we could do with more Mae's."
    f_t "I hope the world will be kind to her."

    nvl clear

    f_t "I put on a pair of baby blue nitrile gloves and load a bucket with toilet cleaning supplies."
    f_t "I have a pack of black nitrile gloves at home too, primarily for sex reasons."
    f_t "I noticed I had Pavlov'd myself into getting hard from wearing these things... A bit problematic when you're trying to clean toilets."

    f "Cleaning sprays, separate wipes for counters and toilets, replacement rolls, trash bags, replacement soap... This should cover it."

    f_t "I make my way to the men's restroom to start off at, it's usually the grossest."
    f_t "An inexplicably wet floor, odd green stains under the toilet seat, nothing out of the ordinary."
    f_t "The women's restroom isn't anything crazy either, except for the distinct smell of used pads and tampons emanating from one of the stalls."

    f "Seems like the cleaners didn't empty a sanitary bin again..."

    f_t "It's a sort of bacterial smell, the metallic note of blood also plays a major role..."
    f_t "..."
    f_t "Why am I talking about this like it's a perfume?"

    nvl clear

    f "Get your head straight... You can get this stuff out of your system this weekend... If you're lucky..."

    f_t "After weeks of sleepless nights I caved and downloaded CRUSHR. Again."
    f_t "After blocking a bunch of creepy chaser types I finally got texted by someone cute..."
    f_t "I suggested midgetgolf, objectively the best first date idea, and she loved it!"
    f_t "I can't wait to meet her..."
    f_t "Her boobs looked so soft..."
    f_t "She's way into stuff I like too..."
    f_t "I never tried or did any of it before but it -sounds- like it'd be awesome..."

    nvl clear

    f "... Oh, yeah, the upstairs toilets."

    f_t "It's not that much of a gender neutral toilet, it's just the third one inexplicably left over."
    f_t "Gendering it would be unfair so it became 'mixed'."
    f_t "I head up the stairs and through the door, I check all the stalls to see if anyone is inside and lock the door to make sure no one disturbs my cleaning."
    f_t "I start with the toilet bowls, the first two go without an issue. However-"

    f "Holy shit."

    nvl clear

    f_t "Piss is everywhere. There's seemingly no surface without the slick-wet yellow sheen of it."
    f_t "The flushing buttons, the toilet brush holder, and-"

    f "Even the fucking toilet roll???"

    f_t "Should I get Mae to clean this? It's kind of her turn..."
    f_t "Like, come on, don't saddle me with this shit."
    f_t "Or did she know about it?"
    f_t "Nonono she's not conniving like that, no way. She's like a golden retriever girl, a golden retriever wouldn't do that."

    f "Hff... Well... I guess I'll do it."

    nvl clear

    f_t "I start by pulling the soaked toilet roll out of the holder, it's completely ruined."
    f_t "A pungent ammonia smell claws its way into my nose and my eyes start to water."

    f "Fuck dude... Drink some water..."

    f_t "After quarantining it in a trash bag I set to work trying to clean the toilet roll holder."
    f_t "It's a sliding contraption that holds two rolls at once. The other one is, thankfully, completely dry."
    f_t "First cleaning off the outside with a wipe and spray, then the inside... The smell already becomes less and less pronounced, instead being overtaken by the sweet-nutty scent of the cleaning agent."
    f_t "One final wiping with some paper towel turns it pristine, mostly thanks to the dire state of its surroundings."

    f "Let's just get this toilet out of the way..."

    nvl clear

    f_t "Dropping to my knees in front of the meticulously amber-misted toilet, I get a moment to inspect the damage."
    f_t "They somehow managed to get pee under the bowl, under the seat, under the lid..."

    f "Very... Thorough..."

    f_t "Here it's where I pick up something new, a scent unlike that of the toilet roll."
    f_t "It's milder, vaguely nutty, a -full- smell. A -healthy- smell."
    f_t "It's not usually my thing, but..."

    nvl clear

    f "Hfffgh..."

    f_t "I'm very much in the mood, and then it very much -is-."
    f_t "I get closer to the toilet bowl and inspect it all around, it really is -covered-."
    f_t "I can make out all the tiny golden droplets desperately sticking to the bowl, some falling by their own weight, catching others on their way down."
    f_t "The growing rolling tear of ochre reaches the foot of the toilet to join a puddle, and it calls to me."

    nvl clear

    piss "You know you want a taste of me, come and get it you nasty bitch."

    nvl clear

    f_t "I unbuckle my belt and hurriedly pull out my cock, my half-hearted tuck has been pressing up against the inseam of my pants for what feels like ages."
    f_t "Feeling it free itself shoots a jolt of pleasure through my hips."
    f_t "I grab a hold of the shaft with my gloved hand and carefully start stroking."
    f_t "I'm so hard it kind of hurts, but the pressure on my foreskin slowly alleviates the wetter I get."
    f_t "I lower my face to the puddle, a eyelash's length removed from touching it directly with my lips."
    f_t "My shallow breaths ripple its surface, my narcissus-yellow reflection floats and warps."
    f_t "Carefully, I place a kiss on its surface. I pull back slightly and let the droplets rest on my lips, acquiring a taste for it."

    nvl clear

    f "Mmh..f...fuck... That's..."
    
    f_t "The flavour is different. Less nutty than it smells, more pungent, it reminds me of kimchi."
    f_t "I feel myself getting close, and dive in for a fully committed lick."
    f_t "I flick the trembling surface with my tongue and catch a full taste."
    f_t "The flavours of it shoot through my mouth, violently bouncing between the mucosa lining of my cheeks."
    f_t "My dick twitches and attention falters together with my balance."

    nvl clear

    f "Wah!!!"

    f_t "I manage to catch myself on the wet toilet bowl, barely not slipping from its slippery porcelain but thoroughly wetting the upper half of my shirt."

    f "W-wah..."

    f "..."

    f "F-for fucks sake..."

    nvl clear
    jump date